...
function StatusBadge({ submission }) {
  const badge = activitySubmissionStatusBadge(submission)
...
export default function TeacherActivityView() {
  const { id } = useParams()
  ...
  const loadData = useCallback(async () => {
    ...
    setLoadError('')
    try {
      const data = await fetchTeacherActivitySubmissions(id)
      setActivity(data.activity)
      setSubmissions(data.submissions)
      if (data.expiredUpdated) {
        toastRef.current.info(FACULTY_MSG.activities.deadlineExpired, {
          toastId: FACULTY_TOAST_ID.activityDeadlineExpired,
          durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
        })
      }
    } catch (e) {
      console.error('[TeacherActivityView]', e)
      setActivity(null)
      setSubmissions([])
    ...
    return [...list].sort((a, b) => {
      ...
      switch (sortKey) {
        case 'no':
          ...
          break
        case 'status':
          av = activitySubmissionStatusBadge(a).label.toLowerCase()
          bv = activitySubmissionStatusBadge(b).label.toLowerCase()
          break
        case 'upload_date':
  ...
  async function saveScore() {
    ...
    if (!Number.isFinite(score) || score < 0 || score > totalScore) {
      toastRef.current.error(`Score must be between 0 and ${totalScore}.`, {
        toastId: FACULTY_TOAST_ID.activityScoreError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
      return
    }
    setSavingScore(true)
    try {
      const updated = await updateActivitySubmissionScore(id, scoreTarget.id, score)
      setSubmissions((prev) => prev.map((s) => (s.id === updated.id ? updated : s)))
      setScoreTarget(null)
      toastRef.current.success(FACULTY_MSG.activities.scoreUpdated, {
        toastId: FACULTY_TOAST_ID.activityScoreSuccess,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
    } catch (e) {
      toastRef.current.error(FACULTY_MSG.activities.scoreUpdateFailed, {
        toastId: FACULTY_TOAST_ID.activityScoreError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
    } finally {
  ...
  const viewUrl = viewTarget ? resolveActivityFileUrl(viewTarget.file_path) : ''
    ...
    <>
      ...
      <main className="min-h-0 flex-1 space-y-8 overflow-y-auto overflow-x-hidden p-4 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <h2 className="text-2xl font-bold text-neutral-900">Activity</h2>
          <BackButton to="/teacher/activities" className="" />
        </div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-neutral-600">Activity not found.</p>
        ) : (
          <>
            <div className="rounded-xl border border-neutral-200 bg-white p-6 shadow-md">
              <h3 className="mb-4 text-base font-bold uppercase text-neutral-900">
                ACTIVITY {activityIndex} Info:
              </h3>
              <table className="w-full border-collapse text-sm">
                <tbody>
                  <tr>
                    <td style={labelStyle}>Title</td>
                    <td style={valueStyle}>{activity.title || '—'}</td>
                    <td style={labelStyle}>Subject</td>
                    <td style={valueStyle}>{activity.subject_name || '—'}</td>
                  </tr>
                  <tr>
                    <td style={labelStyle}>Grade Level</td>
                    <td style={valueStyle}>{activity.grade_level || '—'}</td>
                    <td style={labelStyle}>Upload Date</td>
                    <td style={valueStyle}>{formatDateYmd(activity.created_at)}</td>
                  </tr>
                  <tr>
                    <td style={labelStyle}>Submission Date</td>
                    <td style={valueStyle}>{formatDateYmd(activity.submission_deadline)}</td>
            ...
            <section className="rounded-xl border border-neutral-200 bg-white p-5 shadow-md md:p-6">
              <div className="border-b-2 pb-2" style={{ borderColor: ACTION_BLUE }}>
                <div className="inline-flex items-center gap-2">
                  <h3 className="text-base font-bold text-neutral-800">Activity Submissions</h3>
                ...
                <table className="min-w-full border-collapse text-sm">
                  <thead className="border-b border-neutral-200 bg-neutral-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left">
                        <SortHeader label="Activity Status" active={sortKey === 'status'} direction={sortDir} onClick={() => toggleSort('status')} />
                      </th>
                      <th className="px-4 py-3 text-left">
