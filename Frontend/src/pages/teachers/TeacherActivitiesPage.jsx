...
export default function TeacherActivitiesPage() {
  ...
  const loadActivities = useCallback(
    async (page) => {
      setLoading(true)
      try {
        const result = await fetchTeacherActivities(page, {
        ...
        setTotal(0)
        setTotalPages(1)
        console.error('[TeacherActivitiesPage]', e)
      } finally {
        setLoading(false)
      }
    },
  ...
  useEffect(() => {
    void loadActivities(currentPage)
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true)
    try {
      await deleteTeacherActivity(deleteTarget.id)
      setDeleteTarget(null)
      toastRef.current.success(FACULTY_MSG.activities.deleted, {
        toastId: FACULTY_TOAST_ID.activityDeleteSuccess,
      ...
      if (pageToLoad !== currentPage) {
        ...
        await loadActivities(pageToLoad)
      }
    } catch (e) {
      toastRef.current.error(FACULTY_MSG.activities.deleteFailed, {
        toastId: FACULTY_TOAST_ID.activityDeleteError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
      console.error(e)
    } finally {
      ...
      <main className="min-h-0 flex-1 space-y-6 overflow-y-auto overflow-x-hidden p-4 md:p-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="text-xl font-bold text-neutral-900 md:text-2xl">Activity list</h2>
            <div className="mt-3 inline-flex items-center gap-2 border-b-2 pb-1" style={{ borderColor: ACTION_BLUE }}>
              <span className="text-sm font-semibold text-neutral-800">All Activity</span>
        ...
        <section className="rounded-xl border border-neutral-200 bg-white p-5 shadow-md md:p-6">
          ...
          {loading ? (
            <p className="py-12 text-center text-sm text-neutral-500">Loading activities…</p>
          ) : (
            <>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex flex-wrap gap-2">
                              {!isPastDeadline(row.submission_deadline) ? (
                                <ActionBtn label="Edit" style={BTN_EDIT} onClick={() => navigate(`/teacher/activities/${row.id}/edit`)} />
                              ) : null}
                              <ActionBtn label="Delete" style={BTN_DELETE} onClick={() => setDeleteTarget(row)} />
                              <ActionBtn label="View / Submissions" style={BTN_VIEW} onClick={() => navigate(`/teacher/activities/${row.id}`)} />
                            </div>
                          </td>
                        </tr>
                      ))
      ...
      {deleteTarget ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            <h3 className="text-lg font-bold text-neutral-900">Delete activity</h3>
