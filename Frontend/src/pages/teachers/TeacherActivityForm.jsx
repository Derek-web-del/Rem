...
export default function TeacherActivityForm({ mode = 'add' }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.error('[TeacherActivityForm] options', e)
      }
    })()
    return () => {
      cancelled = true
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const rows = await fetchGradeComponentsForSubject(activeSubjectId, 'activity', {
  ...
  function validate() {
    if (!String(form.title || '').trim()) {
      toastRef.current.error('Activity title is required.', {
    ...
    if (!String(form.subject_name || '').trim()) {
      toastRef.current.error(FACULTY_MSG.activities.subjectRequired, {
        toastId: FACULTY_TOAST_ID.activitySubjectError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
      return false
    }
    if (!String(form.grade_level || '').trim()) {
      toastRef.current.error(FACULTY_MSG.activities.gradeRequired, {
        toastId: FACULTY_TOAST_ID.activityGradeError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
      return false
    }
    ...
    if (!isEdit && !file) {
      toastRef.current.error('Activity file is required.', {
        toastId: FACULTY_TOAST_ID.activityAddError,
        durationMs: FACULTY_ANNOUNCEMENT_TOAST_MS,
      })
      return false
      ...
      if (isEdit) {
        ...
        toastRef.current.error(
          msg.includes('PDF') || msg.includes('Only PDF')
            ...
            : msg || FACULTY_MSG.activities.addFailed,
          {
      ...
      <main className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-neutral-200 pb-4">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold text-neutral-900">Activity</h2>
          </div>
                ...
                <label className="block text-sm font-medium text-neutral-700">
                  ...
                  <div className="relative mt-1">
                    <input
                      ...
                      required
                    />
                    <i className="ti ti-calendar pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400" aria-hidden="true" />
                  </div>
                </label>
                ...
                <label className="block text-sm font-medium text-neutral-700">
                  ...
                  <div className="relative mt-1">
                    <input
                      ...
                      required
                    />
                    <i className="ti ti-clock pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400" aria-hidden="true" />
                  </div>
                </label>
              </div>
            </div>
